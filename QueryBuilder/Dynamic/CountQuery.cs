...
namespace Microsoft.DigitalWorkplace.DigitalTwins.QueryBuilder.Dynamic
{
    ...
    {
        internal CountQuery(string rootTwinAlias, IList<string> allowedAliases, SelectClause select, FromClause fromClause, IList<JoinClause> joins, WhereClause where) : base(rootTwinAlias, allowedAliases, select, fromClause, joins, where)
        {
        }
    }
}